package com.dejun.devops.web.rest;
...
import com.dejun.devops.IntegrationTest;
import com.dejun.devops.config.EmbeddedKafka;
import com.dejun.devops.config.KafkaSseConsumer;
import com.dejun.devops.config.KafkaSseProducer;
...
class JhipsterKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/jhipster-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/jhipster-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/jhipster-kafka/unregister"));
                return;
            }
        }
